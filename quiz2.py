...
bg = PhotoImage(file = "lvl2bg.png")
label2 = Label( root, image=bg)
label2.place(x=0,y=0,relwidth=1,relheight=1)
nextimg=PhotoImage(file="next2.png")
submitimg=PhotoImage(file="sub2.png")
...
def ques():
    global question
    qn1=Label(root,bg="#120A21",fg="white",text=question[question_number-1],width=56,font=("ms sans serif",13),anchor='n')
...
def ansbuttons():
        btn1 = Button(root,text='',borderwidth=0,command=Button1,image=pixel,
          width=211,
          height=50,
          compound="c",font = ("ms sans serif",13))
...
def enable():
    if (question_number)==11:
        ...
        hintb.config(background="#3F1E3D")
...
def question_display():
    ...
    l_list=[]
    for i in range(1,16):
        labe=Label(root,text=i,width=5,bg="#3F1E3D",fg="white")
...
timerl=Label(root, textvariable=timer_label,width=4,font=(6),bg="#AD8AC4")
...
#function for ending quiz program and level unlocking
f=open("level.txt","r")
lvl=f.read(1)
f.close()
def end():
    result()
    if correct_answers>=(data_size*0.8):
        f=open("level.txt","w")
        f.write("3")
        f.close()
    if correct_answers>=(data_size*0.8) and lvl=="2":
        mb.showinfo("Congratulations!","Level 3 is now unlocked")
    home()
...
nextb=Button(root,command=nxt_btn,image=nextimg,borderwidth=0,activebackground="#06030C",bg="#06030C",fg="white")
nextb.place(x=740,y=250)
fiftyb=Button(root,text="50-50",command=fifty,borderwidth=0,activebackground="#3F1E3D",bg="#3F1E3D",fg="white",font=("lucida",12),width=8)
fiftyb.place(x=723,y=100)
hintb=Button(root,text="Hints",command=hints,borderwidth=0,activebackground="#3F1E3D",bg="#E3242B",fg="white",font=("lucida",12),width=8,state="disabled")
hintb.place(x=723,y=134)

#creation of menubar
menubar = Menu(root)
filemenu = Menu(menubar,background="#120A21")
menubar.add_cascade(label='Menu',background="#120A21", menu=filemenu)
filemenu.add_command(label="Home",background="#170C44",foreground="white",command=quithome)
filemenu.add_separator()
filemenu.add_command(label='Instructions',background="#170C44",foreground="white",command=instructions)
filemenu.add_separator()
filemenu.add_command(label='Exit',background="#170C44",foreground="white", command=quit)
